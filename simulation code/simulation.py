#!/usr/bin/env python3
# simulation.py - Simulation and physics calculations with improved reliability
...
class Simulation:
    ...
    def update(self):
        ...
        try:
            ...
            if self.selected_body:
                self.selected_info = self.selected_body.get_info_text()
        except Exception as e:
    ...
    def handle_collisions(self):
        """Detect and handle collisions between bodies - FIXED implementation"""
        ...
        for i in range(len(self.bodies)):
            for j in range(i + 1, len(self.bodies)):
                ...
                # Skip Sun-planet collisions
                if body1.name == "Sun" or body2.name == "Sun":
                    continue
                ...
                # Check for collision
                if r < COLLISION_DISTANCE and body1 != body2:
                    collision_pairs.append((body1, body2))
        
        # Handle collisions after finding all pairs
    ...
    def merge_bodies(self, body1, body2):
        """Merge two bodies preserving momentum"""
        # Make sure both bodies are still in the simulation
        ...
        # Weighted position (center of mass)
        ...
        # Create new name if they're similar size
        if max(body1.mass, body2.mass) / min(body1.mass, body2.mass) < 10:
            new_name = f"{body1.name}-{body2.name} System"
        else:
            new_name = base_body.name
        
        # Create new body with combined properties
        new_radius = (body1.radius**3 + body2.radius**3)**(1/3)  # Volume-based radius
        new_body = Body(
            new_name, 
            total_mass, 
            new_radius / 1000,  # Convert back to km for the constructor
            new_position, 
            new_velocity, 
            base_body.color,
            base_body.texture_name,
            base_body.has_rings
        )
        
        # Check if selected body is being merged
        was_selected = (self.selected_body == body1 or self.selected_body == body2)
        
        # Remove old bodies from the simulation
        ...
        # Update selected body if needed
        if was_selected:
    ...
    def get_info_text(self):
        """Get overall simulation information"""
