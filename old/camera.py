...
class Camera:
    ...
    def handle_mouse_motion(self, pos, rel, buttons):
        ...
        try:
            if buttons[0]:  # Left mouse button - rotation
                ...
                # Scale mouse sensitivity based on distance
                sensitivity = 0.01 * min(1.0, self.distance / 100.0)
                
                # Update camera angles with sensitivity adjustment
                self.x_angle -= dx * sensitivity
                self.y_angle -= dy * sensitivity
    ...
    def handle_mouse_wheel(self, y):
        ...
        try:
            # Validate y value
            if not isinstance(y, (int, float)) or not np.isfinite(y) or abs(y) > 10:
                return
                
            # Adjust zoom factor based on distance for smoother zooming
            base_factor = 0.9 if y > 0 else 1.1
            
            # Apply zoom with smoother scaling at larger distances
            zoom_factor = base_factor ** min(1.0, abs(y))
    ...
    def update_free_movement(self, dt):
        ...
        try:
            ...
            # Apply distance-based scaling for smoother movement at different zoom levels
            distance_scale = np.sqrt(self.distance / 100.0)
            move_amount *= distance_scale
